 ...
 * Modified to keep the maximum error for each bucket
...
{
    size_t bucket = (kmer >> (alpha*k - buckets));
	...
	if(lcp + idx == n || s[lcp] > reference[idx+lcp])
	{
		...
		lo = predicted;
		hi = min(n-1, predicted+maxOvers[bucket]); // Over-prediction which the actual position is highly likely to not exceed
	...
	else
	{
		...
		lo = (size_t)max(0LL, (long long)predicted-maxUnders[bucket]);
		hi = predicted;
		size_t loIdx = rev[lo];
...
void buildPiecewiseLinear(string& s, vector<size_t> sa)
{
	...
	maxOvers = new int[(1L<<buckets)+1];
	maxUnders = new int[(1L<<buckets)+1];
	size_t n = overs.size() + unders.size();
	for(size_t i = 0; i<n; i++)
	{
	    size_t bucket = (xs[i] >> (alpha*k - buckets));
		maxUnders[bucket] = (int)max(-errors[i], maxUnders[bucket]);
		maxOvers[bucket] = (int)max(errors[i], maxOvers[bucket]);
	}
}
...
int main(int argc, char **argv)
{
    ...
    if(saplingf.good())
    {
        ...
        maxOvers = new int[xlistsize];
        maxUnders = new int[xlistsize];
        fread(&xlist[0], sizeof(long long), xlistsize, infile);
        fread(&ylist[0], sizeof(long long), xlistsize, infile);
        fread(&maxOvers[0], sizeof(int), xlistsize, infile);
        fread(&maxUnders[0], sizeof(int), xlistsize, infile);
    }
    else
    {
        ...
        fwrite(&maxOvers[0], sizeof(int), xlistsize, outfile);
        fwrite(&maxUnders[0], sizeof(int), xlistsize, outfile);
    }
