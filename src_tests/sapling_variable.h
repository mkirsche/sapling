...
struct Sapling {
    ...
    long long kmerize(int curK, const string& s)
    {
        long long kmer = 0;
	    for(int i = 0; i<curK; i++) kmer = (kmer << alpha) | vals[s[i]];
	    return kmer;
    }
    
    size_t queryPiecewiseLinear(long long x)
    {
	    size_t bucket = (x >> (alpha*k2 - buckets));
	    ...
	    long long predict = (long long)(.5 + ylo + (yhi - ylo) * (x * 1.0 / (1<<(alpha*(k2-k))) - xlo) * 1. / (xhi - xlo));
    ...
    void buildPiecewiseLinear(string& s, vector<size_t> sa)
    {
	    vector<long long> xs;
	    vector<size_t> ys;
	    long long hash = kmerize(k, s.substr(0, k));
	    ...
	    overs.resize(0);
	    unders.resize(0);
	    FILE *outfile = fopen ("errorList_variable.out", "w");
	    fprintf(outfile, "x\ty\tpred\terror\n");
	    hash = kmerize(k2, s.substr(0, k2));
	    for(size_t i = 0; i+k2<=s.length(); i++)
	    {
		    size_t x = hash;
		    size_t y = sa[i];
		    size_t predict = queryPiecewiseLinear(x);
		    errors[i] = getError(y, predict);
		    fprintf(outfile, "%zu\t%zu\t%zu\t%d\n", x, y, predict, errors[i]);
		    if(errors[i] > 0) overs.push_back(errors[i]);
		    else unders.push_back(-errors[i]);
		    hash &= (1L << (2*(k2-1))) - 1;
		    hash <<= 2;
		    if(i + k2 < s.length()) hash |= vals[s[i+k2]];
	    }
	    errorStats();
    }
    ...
    Sapling(string refFnString)
    {
        ...
        string saplingfnString = refFnString + ".sap_variable";
