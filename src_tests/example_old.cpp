...
int main(int argc, char **argv)
{
    ...
    {
    	size_t idx = rand() % (sap.n - sap.k2);
    	queries[i] = sap.reference.substr(idx, sap.k2);
    	kmers[i] = sap.kmerize(sap.k2, queries[i]);
    }
    ...
    for(int i = 0; i < numQueries; i++)
    {
        ...
        for(int j = 0; j<sap.k2; j++) fprintf(outfile, "9");
        fprintf(outfile, "\n");
    }
    ...
    cout << "Sapling time: " << elapsed_seconds.count() << endl;
    
    // Check the answers
    int countCorrect = 0;
    for(int i = 0; i<numQueries; i++)
    {
        if(plAnswers[i] + sap.k2 <= sap.n && queries[i] == sap.reference.substr(plAnswers[i], sap.k2))
        {
            countCorrect++;
        }
    }
